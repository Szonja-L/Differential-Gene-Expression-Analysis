...
print("Dataset: GSE7904 (real NCBI GEO data) — Tumor vs. Normal Breast")
print("=" * 60)
...
print(f"\n--- SUMMARY (named genes only — uncharacterized 'LOC' loci excluded) ---")
print(f"Total named genes tested: {len(real_genes)}")
print(f"  (raw probe-collapsed total incl. LOC loci: {len(results_df)})")
print(f"Significant DEGs (adj.p<0.05, |log2FC|>=1): {real_genes['significant'].sum()}")
print(f"  Upregulated in tumor:   {(real_genes['direction']=='Upregulated in Tumor').sum()}")
print(f"  Downregulated in tumor: {(real_genes['direction']=='Downregulated in Tumor').sum()}")
...
# Save results
results_df.to_csv('results/dge_results_all.csv', index=False)
real_genes.to_csv('results/dge_results_genes.csv', index=False)
real_genes[real_genes['significant']].to_csv('results/dge_significant.csv', index=False)
